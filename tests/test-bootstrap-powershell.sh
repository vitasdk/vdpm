...
mkdir -p "$seed/bin" "$seed/usr/bin" "$seed/usr/ssl/certs" "$seed/share/vdpm"
cp "$repository/share/vdpm/channel-public-key.pem" "$seed/share/vdpm/"
: >"$seed/usr/bin/msys-2.0.dll"
: >"$seed/usr/ssl/certs/ca-bundle.crt"
...
cat >"$seed/usr/bin/vdpm-channel.exe" <<'STUB'
#!/usr/bin/env bash
exit 0
STUB
...
cat >"$seed/usr/bin/pacman.exe" <<'STUB'
#!/usr/bin/env bash
set -euo pipefail
if [[ ${MSYS:-} != *noglob* ]]; then
	expanded=()
...
STUB
chmod +x "$seed/bin/vdpm.exe" "$seed/usr/bin/pacman.exe" "$seed/usr/bin/vdpm-channel.exe"
...
decoy=$work/cwd
mkdir -p "$decoy/here/bin" "$decoy/here/usr/bin" "$decoy/here/share/vdpm" "$decoy/here/etc"
: >"$decoy/here/bin/vdpm.exe"
: >"$decoy/here/etc/pacman.conf"
# More than one file per directory, as the staged bundle has: a pattern that
# expands to a single name is swallowed by its own --overwrite and hides the
# problem, which is what the first version of this test did.
for name in pacman.exe vdpm-channel.exe msys-2.0.dll; do
	: >"$decoy/here/usr/bin/$name"
done
...
check "with the patterns it was given" \
	"$(grep -c '^\*/' "$work/pacman-arguments.txt" || true)" 4
